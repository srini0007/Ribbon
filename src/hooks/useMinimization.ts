...
const traverse = (child: any, arr: any[]) => {
  if (
    child.classList.contains("radio-group") ||
    child.classList.contains("tabSubGroup-min") ||
    child.classList.contains("check-group") ||
    child.classList.contains("ribbon-icon-button") ||
    child.classList.contains("dropdown") ||
    child.classList.contains("ribbon-tool-button")
  ) {
    arr.push(child);
      ...
      menuRef.current.childNodes.forEach((childeNode: ChildNode) => {
        if (childeNode instanceof HTMLElement) {
          ...
          if (excess === null) {
            ...
            if (
              aggWidth >= OVERFLOWED - 1000 ||
              aggWidth + ele.offsetWidth > range
            ) {
              ...
              if (hasElement) {
                // if its true then there is one element that has been overflowed

                partialOver.push(
                  childeNode.children[
                    childeNode.childElementCount - 1
                  ] as HTMLElement
                ); // adding tab group title
              }
              // portalElements.push(childeNode);
